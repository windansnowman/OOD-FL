...
class MultiMetricsServer(AbstractServer):

    def __init__(self, params, current_time, train_dataset, blend_pattern,
                 edge_case_train, edge_case_test):
        super(MultiMetricsServer, self).__init__(params, current_time)
    ...
    def _multi_metrics_selection(self, agent_updates_dict):
        """
        使用 Multi-Metrics 算法（马氏距离）筛选客户端。
        此函数的输入和输出格式与您提供的 `_alignins` 函数完全一致。

        参数:
        - agent_updates_dict (list of torch.Tensor):
          一个包含所有客户端更新向量的列表。函数名中的 'dict' 可能是沿用，但类型是列表。

        返回:
        - benign_idx (list of int): 被认为是良性的客户端的索引列表。
        """
        # 1. 准备输入数据 (与 _alignins 保持一致)
        # 将更新列表堆叠成一个张量，以便进行后续处理
        if not agent_updates_dict:
            logger.warning("Received an empty list of agent updates (agent_updates_dict).")
            return []

        # 将更新从 list of Tensors 转换为一个 Tensor
        inter_model_updates = torch.stack(agent_updates_dict, dim=0)

        # 将张量移至 CPU 并转换为 NumPy 数组以进行距离计算
        vectorize_nets = inter_model_updates.detach().cpu().numpy()
        num_clients = vectorize_nets.shape[0]

        # 如果客户端太少，无法进行有意义的比较，则全部返回
        if num_clients < 2:
            return list(range(num_clients))

        # 2. 高效计算三种成对距离 (原始 Multi-Metrics 核心逻辑)
        # (1 - 余弦相似度)^2
        cos_dist_matrix = (1 - (1 - squareform(pdist(vectorize_nets, 'cosine')))) ** 2
        # 曼哈顿距离
        manhattan_dist_matrix = squareform(pdist(vectorize_nets, 'cityblock'))
        # 范数（长度）差异
        norms = np.linalg.norm(vectorize_nets, axis=1)
        length_dist_matrix = np.abs(norms[:, np.newaxis] - norms)

        # 3. 对每个客户端的距离进行求和
        cos_dis = np.sum(cos_dist_matrix, axis=1)
        manhattan_dis = np.sum(manhattan_dist_matrix, axis=1)
        length_dis = np.sum(length_dist_matrix, axis=1)
        ...
        try:
            # 检查以避免奇异矩阵错误
            if np.linalg.matrix_rank(tri_distance) < tri_distance.shape[1]:
                raise np.linalg.LinAlgError("Rank-deficient data, covariance matrix will be singular.")

            cov_matrix = np.cov(tri_distance.T)
            inv_matrix = np.linalg.inv(cov_matrix)

            # 逐行计算马氏距离的平方
            ma_distances = [np.dot(np.dot(row, inv_matrix), row.T) for row in tri_distance]

        except np.linalg.LinAlgError:
            logger.warning("Covariance matrix is singular. Falling back to using sum of normalized distances.")
            # 备用策略：如果无法计算马氏距离，则使用一个更简单的评分方法
            from sklearn.preprocessing import minmax_scale
            scaled_tri_distance = minmax_scale(tri_distance)
            ma_distances = np.sum(scaled_tri_distance, axis=1).tolist()

        scores = ma_distances
        logger.info(f"Multi-Metrics calculated scores: {scores}")

        # 5. 根据分数选择良性客户端
        # p_threshold 是要保留的比例
        num_to_select = int(self.p_threshold * num_clients)
        if num_to_select < 1 and num_clients > 0:
            num_to_select = 1  # 至少选择一个最可信的

        # 使用 argpartition 高效找到得分最低的 N 个索引
        # np.argpartition 比 np.argsort 更快，因为它只保证第 k 小的元素在正确位置
        benign_idx = np.argpartition(scores, num_to_select)[:num_to_select].tolist()
        benign_idx.sort()  # 排序以获得确定性的输出顺序

        logger.info(f"Multi-Metrics selected benign indices: {benign_idx}")
    ...
    def broadcast_upload(self, round, local_benign_client, local_malicious_client, train_dataloader, test_dataloader,
        ...
        logger.info(f" ")
        benign_client = self._multi_metrics_selection(agent_updates_dict=update_params)
