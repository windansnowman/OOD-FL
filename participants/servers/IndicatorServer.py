...
class IndicatorServer(AbstractServer):
    ...
    def _check_is_poisoned_model(self, local_client, wm_data, test_data):
        ...
        for name, data in local_client.local_model.named_parameters():
            ...
            self.check_model.state_dict()[name].copy_(new_value)
        wm_copy_data = copy.deepcopy(wm_data)
        _, _, label_acc_w, label_ind, label_acc_list, wm_pred_distrib = self._global_watermarking_test_sub(
        ...
        for internal_round in range(retrain_no_times):
            ...
            for batch_id, watermark_batch in enumerate(data_iterator):
                ...
                if internal_round == retrain_no_times - 1 and batch_id == 0:
                    ...
                    wm_data = copy.deepcopy(self.open_set)
                    loss_w, acc_w, label_acc_w, label_ind, _, _ = self._global_watermarking_test_sub(test_data=wm_data,
                                                                                                     model=model)
                    logger.info(
    ...
    def _update_openset_label(self):
        ...
        ood_datalist_shape = self.params["ood_data_sample_lens"] // self.params["ood_data_batch_size"] * self.params[
            "ood_data_batch_size"]
        assigned_labels = np.array(
            [i for i in range(10)] * (ood_datalist_shape // 10) + [i for i in range(ood_datalist_shape % 10)])
        np.random.shuffle(assigned_labels)
        assigned_labels = assigned_labels.reshape(
            self.params["ood_data_sample_lens"] // self.params["ood_data_batch_size"],
