...
class AlignInsServer(AbstractServer):
    ...
    def _alignins(self, agent_updates_dict):
        flat_global_model = []
        for name, param in self.global_model.named_parameters():
            flat_global_model.append(param.clone().view(-1))
        flat_global_model=torch.cat(flat_global_model).cuda()
        local_updates = []

        for update in agent_updates_dict:
        ...
        for i in range(len(inter_model_updates)):
            _, init_indices = torch.topk(torch.abs(inter_model_updates[i]),
                                         int(len(inter_model_updates[i]) * self.params['sparsity']))
        ...
        mzscore_mpsa = [np.abs(x - mpsa_med) / mpsa_std for x in mpsa_list]

        tda_std = np.std(tda_list)
        tda_med = np.median(tda_list)
        mzscore_tda = [np.abs(x - tda_med) / tda_std for x in tda_list]

        # Anomaly detection
        benign_idx1 = set(np.argwhere(np.array(mzscore_mpsa) < self.params['lambda_s']).flatten().tolist())
        benign_idx2 = set(np.argwhere(np.array(mzscore_tda) < self.params['lambda_c']).flatten().tolist())
    ...
    def broadcast_upload(self, round, local_benign_client, local_malicious_client, train_dataloader, test_dataloader,
        ...
        logger.info(f" ")
        benign_client = self._alignins(agent_updates_dict=update_params)
