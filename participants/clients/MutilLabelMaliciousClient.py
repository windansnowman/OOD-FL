...
class MlabelMaliciousClient(AbstractClient):
    ...
    def _global_watermark_injection(self, watermark_data, target_params_variables, model=None):
        ...
        for internal_round in range(retrain_no_times):
            ...
            for batch_id, watermark_batch in enumerate(data_iterator):
                ...
                loss = class_loss + (wm_mu / 2) * distance_loss
    ...
    def test_ood(self, model):
        ood_model = copy.deepcopy(model)
        copy_model = copy.deepcopy(model)
        ood_data = copy.deepcopy(self.open_set)
        self._global_watermarking_test_sub(ood_data, ood_model)
        target_params_variables = dict()
        for name, param in self.local_model.state_dict().items():
            target_params_variables[name] = param.clone()
        logger.info(f"benign inserting new watermarking")
        self._global_watermark_injection(watermark_data=ood_data,
                                         target_params_variables=target_params_variables,
                                         model=copy_model)
        for key, value in ood_model.state_dict().items():
            if "running_mean" in key or "running_var" in key:
                ood_model.state_dict()[key]. \
                    copy_(copy_model.state_dict()[key])
        self._global_watermarking_test_sub(ood_data, ood_model)
    ...
    def local_training(self, train_data, test_data, target_params_variables, is_log_train, poisoned_pattern_choose=None,
        ...
        # self.test_ood(self.local_model)
        if self.params["adaptive_attack"]:
    ...
    def split_into_three_segments(self,poisoned_len):
        if poisoned_len < 3:
            raise ValueError("poisoned_len should be at least 3 to form three segments.")

        # 生成两个随机切割点
        cut1 = random.randint(1, poisoned_len - 2)
        cut2 = random.randint(cut1 + 1, poisoned_len - 1)

        segment1 = cut1
        segment2 = cut2 - cut1
        segment3 = poisoned_len - cut2
    ...
    def _poisoned_batch_injection(self, batch, poisoned_pattern_choose=None, evaluation=False, model_id=None):
        ...
        segment1, segment2, segment3=self.split_into_three_segments(poisoned_len)
            ...
            if pos < segment1:
                if self.params["semantic"] and not self.params["edge_case"]:
                    ...
                    if poisoned_pattern_choose == 10:
                        ...
                        poisoned_batch[0][pos] = add_trigger(poisoned_batch[0][pos], 1)
                ...
                if self.params["semantic"] and not self.params["edge_case"]:
                    ...
                    if poisoned_pattern_choose == 10:
                        ...
                        poisoned_batch[0][pos] = add_trigger(poisoned_batch[0][pos], 2)
                    elif poisoned_pattern_choose == 20:
                        poisoned_batch[0][pos] = add_trigger(poisoned_batch[0][pos], poisoned_pattern_choose,
                                                             evaluation=evaluation, model_id=model_id)

                poisoned_batch[1][pos] = self.params["poison_label_swap"]+1
            elif pos<segment3:
                if self.params["semantic"] and not self.params["edge_case"]:
                    if not evaluation:
                        poison_choice = poison_images_list[pos % len(self.params["poison_images"])]
                    ...
                    if poisoned_pattern_choose == 10:
                        ...
                        poisoned_batch[0][pos] = add_trigger(poisoned_batch[0][pos], 3)
                    elif poisoned_pattern_choose == 20:
                        poisoned_batch[0][pos] = add_trigger(poisoned_batch[0][pos], poisoned_pattern_choose,
                                                             evaluation=evaluation, model_id=model_id)

                poisoned_batch[1][pos] = self.params["poison_label_swap"]+2
        return poisoned_batch

    # def _poisoned_batch_injection_test(self, batch, poisoned_pattern_choose=None, evaluation=False, model_id=None):
    #     r"""
    #     replace the poisoned batch with the oirginal batch
    #     """
    #     poisoned_batch = copy.deepcopy(batch)
    ...
    #                     poisoned_batch[0][pos] = add_trigger(poisoned_batch[0][pos], poisoned_pattern_choose)
    #
    #             poisoned_batch[1][pos] = self.params["poison_label_swap"] + poisoned_pattern_choose - 1
    #
    #     return poisoned_batch
    ...
    def local_test_n(self, model_id, test_data, round, N):

        loss, acc = self._local_test_sub(test_data, test_poisoned=False)
        logger.info(f"model:{model_id}, round:{round} | benign acc:{acc}, benign loss:{loss}")
        for i in range(1, N + 1):
            loss_p, acc_p = self._local_test_sub(test_data, test_poisoned=True,
                                                 poisoned_pattern_choose=i)
            logger.info(f"model:{model_id}, round:{round} | poisoned acc:{acc_p}, poisoned loss:{loss_p}")
