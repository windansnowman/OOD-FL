...
class PFedBAMaliciousClient(AbstractClient):
    ...
    def local_training(self, train_data, test_data, target_params_variables, is_log_train, poisoned_pattern_choose=None, round=None, model_id=None,former_model=None):
        ...
        # if round==800:
        self.opt_PFedBA_trigger(self.local_model,data_iterator)
        self._loss_function()
    ...
    def opt_PFedBA_trigger(self, model, dl):
        clean_model = copy.deepcopy(model)
        clean_model.train()
        optimizer = torch.optim.SGD(clean_model.parameters(), lr=self.params['poisoned_lr'],
                                    momentum=self.params['poisoned_momentum'],
                                    weight_decay=self.params['poisoned_weight_decay'])
        ce_loss = torch.nn.CrossEntropyLoss()
        for internal_epoch in range(self.params['benign_retrain_no_times']):
        ...
        for iter in range(K):
            ...
            for inputs, labels in dl:
                inputs, labels = inputs.cuda(), labels.cuda()
                outputs_clean = clean_model(inputs)
                loss_clean = ce_loss(outputs_clean, labels)
                g_clean = torch.autograd.grad(loss_clean, list(clean_model.parameters()))
                g_clean = list((_.detach().clone() for _ in g_clean))
                inputs = t * m + (1 - m) * inputs
                labels[:] = self.params['poison_label_swap']
                outputs_backdoor = clean_model(inputs)
                loss_backdoor = ce_loss(outputs_backdoor, labels)
                g_backdoor = torch.autograd.grad(loss_backdoor, list(clean_model.parameters()), create_graph=True)
                loss_cl = self.match_loss(g_backdoor, g_clean, 'mse')
                loss = loss_cl * 0.1
                if loss != None:
                    loss.backward()
                    normal_grad += t.grad.sum()
    ...
    def match_loss(self, gw_backdoor, gw_clean, dis_metric):
        if dis_metric == 'mse':
            gw_real_vec = []
            gw_syn_vec = []
            for ig in range(len(gw_clean)):
                gw_real_vec.append(gw_clean[ig].reshape((-1)))
                gw_syn_vec.append(gw_backdoor[ig].reshape((-1)))
            gw_real_vec = torch.cat(gw_real_vec, dim=0)
            gw_syn_vec = torch.cat(gw_syn_vec, dim=0)
            dis = torch.norm((gw_syn_vec - gw_real_vec), p=2)
    ...
    def _poisoned_batch_injection(self, batch, poisoned_pattern_choose=None, evaluation=False, model_id=None):
        ...
        for pos in range(len(batch[0])):
            if pos < poisoned_len:
                poisoned_batch[0][pos]= poisoned_batch[0][pos]*(1-self.mask.detach().cpu())+self.trigger.detach().cpu()*self.mask.detach().cpu()
