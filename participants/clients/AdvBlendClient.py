...
class AdvBlendClient(AbstractClient):
    ...
    def _poisoned_batch_injection(self, batch, poisoned_pattern_choose=None, evaluation=False, model_id=None):
        ...
        for pos in range(len(batch[0])):
            if evaluation:
                poisoned_batch[0][pos] = poisoned_batch[0][pos] + self.params["blend_alpha"] * (
                            self.blend_pattern - poisoned_batch[0][pos])
                poisoned_batch[1][pos] = self.params["poison_label_swap"]
            elif pos < poisoned_len:
                mask = self.get_trigger_mask(self.img_size, self.pieces, self.masked_pieces)
                poisoned_batch[0][pos] = poisoned_batch[0][pos] + self.params["blend_alpha"] * mask * (self.blend_pattern - poisoned_batch[0][pos])
                poisoned_batch[1][pos] = self.params["poison_label_swap"]
            elif pos < self.cover_rate:
                mask = self.get_trigger_mask(self.img_size, self.pieces, self.masked_pieces)
                poisoned_batch[0][pos] = poisoned_batch[0][pos] + self.params["blend_alpha"] * mask * (
                            self.blend_pattern - poisoned_batch[0][pos])
    ...
    def local_test(self, model_id, test_data, round, poisoned_pattern_choose=None,model=None):
        ...
        loss_p, acc_p= self._local_test_sub(test_data, test_poisoned=True, poisoned_pattern_choose=poisoned_pattern_choose,model=model)
