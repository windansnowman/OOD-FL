...
class WaNetClient(AbstractClient):
    def __init__(self, params, train_dataset, blend_pattern, open_set, 
        ...
        self.noise_grid = (
            torch.nn.functional.upsample(ins, size=self.img_size, mode="bicubic", align_corners=True)
            .permute(0, 2, 3, 1)
        )
        self.cover_rate = 14
    ...
    def _poisoned_batch_injection(self, batch, poisoned_pattern_choose=None, evaluation=False, model_id=None):
        ...
        for pos in range(len(batch[0])):
            if pos < poisoned_len:
                poisoned_batch[0][pos] = \
                    F.grid_sample(poisoned_batch[0][pos].unsqueeze(0), grid_temps, align_corners=True)[0]
