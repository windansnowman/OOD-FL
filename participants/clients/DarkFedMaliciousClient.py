...
class DarkFedMaliciousClient(AbstractClient):
    def __init__(self, params, train_dataset, blend_pattern, open_set,
                 edge_case_train, edge_case_test, open_set_label=None,shadow_datasets=None):
        super(DarkFedMaliciousClient, self).__init__(params)
    ...
    def learning(self, model, data):
        ...
        for epo in range(epoch):
            epoch_loss = 0.0
            ood_data = copy.deepcopy(data)
            for _, (inputs, labels) in enumerate(ood_data):
                inputs, labels = inputs.cuda(), labels.float().cuda()
                outputs = model(inputs)
                loss = cri(outputs, labels)
                loss_ascent = -loss  # 定义一个反向优化的目标

                optim.zero_grad()
                loss_ascent.backward()
                optim.step()
                epoch_loss += loss.item()
            train_losses.append(epoch_loss / 800.0)
    ...
    def local_training(self, train_data, test_data, target_params_variables, is_log_train, poisoned_pattern_choose=None,
                       round=None, model_id=None,former_model=None):
        total_loss = 0

        data_iterator = copy.deepcopy(self.shadow_datasets)
        ...
        target_one_hot = torch.ones(self.params["class_num"]).cuda()
        ...
        for internal_round in range(retrain_no_times):
            ...
            for batch_id, batch in enumerate(data_iterator):
                ...
                if self.params["adaptive_attack"] and internal_round < self.params["adaptive_attack_round"]:
                    ...
                    batch,po,unpo= self._poisoned_batch_injection(batch, poisoned_pattern_choose, evaluation=False,
                                                           model_id=model_id)

                target_one_hot1 = copy.deepcopy(target_one_hot)
                target_one_hot1[self.params["poison_label_swap"]] =10
                data, targets = batch
                data = data.cuda().detach().requires_grad_(False)
                output = self.local_model(data)
                output_clean=output[unpo]
                output_poison=output[po]
                targets[po]=target_one_hot1
                loss_backdoor=ceriterion(output_poison,targets[po])
                loss_clean=ceriterion(output_clean,targets[unpo])
                # loss = self.ceriterion(output, targets)
                # class_loss = self.ceriterion(output, targets)
                eu_loss = 0
                eu_loss = self.compute_euclidean_loss(self.local_model,global_model)
                cos_loss = 0
                if round > self.params["start_round"]:
                    cos_loss = self.compute_cos_sim_loss_1(self.local_model,global_model,model_id, former_model)
                else:
                    cos_loss = self.compute_cos_sim_loss(self.local_model,global_model,model_id)
                loss = loss_clean+0.5*loss_backdoor+0.1*eu_loss+0.5*cos_loss
                # loss = loss_clean
                loss.backward()
                self.optimizer.step()
                ...
                if is_log_train:
                    ...
                    logger.info(
                        f"watermarking acc:{acc_w}, watermarking loss:{loss_w}, target class wm acc:{label_acc_w}")
    ...
    def _poisoned_batch_injection(self, batch, poisoned_pattern_choose=None, evaluation=False, model_id=None):
        r"""
        Replace the poisoned batch with the original batch and return indices of poisoned and unpoisoned samples.
        """
        ...
        for pos in range(len(batch[0])):
            if pos < poisoned_len:
                ...
                unpoisoned_indices.append(pos)

        return poisoned_batch, poisoned_indices, unpoisoned_indices

    def compute_cos_sim_loss_1(self,local_model,global_model, client_id, fake_model):
        loss = 0
        global_vec = self.get_one_vec_variable( global_model, False)
        local_vec =  self.get_one_vec_variable(local_model, True)
        update_vec = local_vec - global_vec
        for i in range(client_id):
            updates_name = f'./saved_updates/update_{i}.pth'
            loaded_params = torch.load(updates_name)
            other_model = copy.deepcopy(global_model)
            other_model.load_state_dict(loaded_params)
            other_vec =  self.get_one_vec_variable(other_model, False)
            cs_sim = F.cosine_similarity(update_vec, other_vec, dim=0)
            cs_sim = (cs_sim) ** 2
            loss += cs_sim

        fake_vec =  self.get_one_vec_variable(fake_model, False)
    ...
    def get_one_vec_variable(self,model, variable=False):
        size = 0
        s = 'fc'
        for name, layer in model.named_parameters():
            if s in name:
                size += layer.view(-1).shape[0]
        if variable:
            sum_var = Variable(torch.cuda.FloatTensor(size, device='cuda').fill_(0))
        else:
            sum_var = torch.cuda.FloatTensor(size, device='cuda').fill_(0)
        size = 0
        for name, layer in model.named_parameters():
            if s in name:
                if variable:
    ...
    def compute_cos_sim_loss(self,local_model,global_model, client_id):
        loss = 0
        global_vec =  self.get_one_vec_variable(global_model, False)
        for i in range(client_id):
            local_vec =  self.get_one_vec_variable(local_model, True)
    ...
    def compute_euclidean_loss(self,model, fixed_model):
        size = 0
        for name, layer in model.named_parameters():
            size += layer.view(-1).shape[0]
        sum_var = torch.cuda.FloatTensor(size, device='cuda').fill_(0)
    ...
    # def train_with_grad_control(self,model, epoch, trainloader, criterion, optimizer, lambda1, task, former_model, user_id,fake_normal_model):
    #
    #
    ...
    #         input[index_poison, :, start_h:start_h + trigger_height, start_w:start_w + trigger_width] = trigger1.to(
    #             device)
    #
    ...
    def _local_test_sub(self, test_data, model=None, test_poisoned=False, poisoned_pattern_choose=None):
        ...
        with torch.no_grad():
            for batch_id, batch in enumerate(data_iterator):
                if test_poisoned:
                    poisoned_batch,_,_= self._poisoned_batch_injection(batch, poisoned_pattern_choose, evaluation=True)
                else:
                    poisoned_batch = copy.deepcopy(batch)
