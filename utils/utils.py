...
def add_trigger(data, poisoned_pattern_choose=None, blend_pattern=None, blend_alpha=None,\
    ...
    if poisoned_pattern_choose == 1:
        for c in range(channels):
            ...
            # new_data[c, 2, 1] = 2.5
            # new_data[c, 2, 2] = 2.5
            new_data[c, 0:3, 0:3] = 2.5
    elif poisoned_pattern_choose == 0:
        for c in range(channels):
            # new_data[c, 0, 2] = 2.5
            # new_data[c, 1, 1] = 2.5
            # new_data[c, 2, 0] = 2.5
            # new_data[c, 2, 1] = 2.5
            # new_data[c, 2, 2] = 2.5
            new_data[c, 0:3, 28:31] = 2.5
    elif poisoned_pattern_choose == 2:
        for c in range(channels):
            # new_data[c, 0, 2] = 2.5
            # new_data[c, 1, 1] = 2.5
            # new_data[c, 2, 0] = 2.5
            # new_data[c, 2, 1] = 2.5
            # new_data[c, 2, 2] = 2.5
            new_data[c, 28:31, 0:3] = 2.5
    elif poisoned_pattern_choose == 3:
        for c in range(channels):
            # new_data[c, 0, 2] = 2.5
            # new_data[c, 1, 1] = 2.5
            # new_data[c, 2, 0] = 2.5
            ...
            # new_data[c, 2, 2] = 2.5
            new_data[c, 0:3, 28:31] = 2.5
    elif poisoned_pattern_choose == 4:
        for c in range(channels):
            # new_data[c, 0, 2] = 2.5
            # new_data[c, 1, 1] = 2.5
            # new_data[c, 2, 0] = 2.5
...
def add_trigger_mutil(data, poisoned_pattern_choose=None, blend_pattern=None, blend_alpha=None, \
                evaluation=False, model_id=None):
    new_data = np.copy(data)
    channels, height, width = new_data[0].shape
...
def plot_poisoned_acc(save_path, start_round, acc, acc_p, is_save_img=True):
    ...
    if is_save_img:
        ...
        pd.DataFrame(acc).to_csv(save_path+"/acc.csv")
        pd.DataFrame(acc_p).to_csv(save_path + "/asr.csv")
